...
function TypingDNA() {
  if (TypingDNA.initialized !== true) {
    ...
    const keyCodesLen = TypingDNA.keyCodes.length;
    ...
    TypingDNA.keyDown = function (e) {
      ...
      const t0 = TypingDNA.pt1;
      TypingDNA.pt1 = new Date().getTime();
      const seekTotal = TypingDNA.pt1 - t0;
      const startTime = TypingDNA.pt1;
      if (
        TypingDNA.recording === true ||
    ...
    TypingDNA.keyPress = function (e) {
      ...
      if (TypingDNA.diagramRecording === true) {
        const keyCode = TypingDNA.dlastDownKey;
      ...
      if (TypingDNA.diagramRecording === true) {
        if (
          ...
          if (TypingDNA.dwfk[keyCode] === 1) {
            ...
            const realKeyCode = TypingDNA.drkc[keyCode];
            const arrD = [
              keyCode,
              seekTime,
              pressTime,
    ...
    TypingDNA.MobileKeyUp = function (e) {
      ...
      const t0 = TypingDNA.ut1;
      TypingDNA.ut1 = new Date().getTime();
      const seekTime = TypingDNA.ut1 - t0;
      const kpGet = TypingDNA.kpGetAll();
      let pressTime = kpGet[0] !== 0 ? Math.round(TypingDNA.ut1 - kpGet[0]) : 0;
      if (isNaN(pressTime)) {
        pressTime = 0;
      }
      const keyCode = e.keyCode;
    ...
    TypingDNA.AndroidKeyUp = function (e) {
      ...
      let targetIndex = TypingDNA.ACInputLengths.inputs.indexOf(e.target);
      if (targetIndex === -1) {
        TypingDNA.ACInputLengths.inputs.push(e.target);
        TypingDNA.ACInputLengths.lastLength.push(0);
      ...
      if (
        ...
        const char = e.target.value.substr(e.target.selectionStart - 1 || 0, 1);
    ...
    TypingDNA.mouseUp = function (e) {
      if (TypingDNA.mouseRecording === true) {
        if (e.which === 1) {
          const clickTime = new Date().getTime() - TypingDNA.lastMouseDownTime;
    ...
    TypingDNA.isTarget = function (target) {
      if (TypingDNA.lastTarget === target && TypingDNA.lastTargetFound) {
        ...
        const targetLength = TypingDNA.targetIds.length;
    ...
    TypingDNA.deviceMotionHandler = function (e) {
      ...
      const kpCurAccX = Math.round(100 * e.accelerationIncludingGravity.x);
      const kpCurAccY = Math.round(100 * e.accelerationIncludingGravity.y);
      const kpCurAccZ = Math.round(100 * e.accelerationIncludingGravity.z);
      if (e.rotationRate) {
    ...
    TypingDNA.kpADifArr = function (arr) {
      ...
      for (i = 0; i < length; i++) {
        const newVal = firstArr[i + 1] - firstArr[i];
        newArr.push(newVal);
    ...
    TypingDNA.kpGetAll = function () {
      ...
      if (TypingDNA.kpAccZ.length < 2) {
        ...
        TypingDNA.kpAccZ.shift();
        TypingDNA.kpTimes.shift();
        let kpPos = kpxPos.concat(kpyPos);
        kpPos = kpPos.sort();
        const kpxyPos = [];
        ...
        let lastKpza = 0;
        let lastKpTime = TypingDNA.kpTimes[TypingDNA.kpTimes.length - 1];
        for (i = 0; i < kpxyPos.length; i++) {
          const j = kpxyPos[i];
          const minj = kpzaAbs.length > 8 ? 2 : kpzaAbs.length > 4 ? 1 : 0;
    ...
    TypingDNA.mouse.getTotalDistance = function (xPositions, yPositions) {
      let totalDistance = 0;
      const length = xPositions.length;
      for (i = 1; i < length - 1; i++) {
        const a = xPositions[i] - xPositions[i - 1];
        const b = yPositions[i] - yPositions[i - 1];
    ...
    TypingDNA.mouse.getAngle = function (xPosDelta, yPosDelta) {
      let angle = 0;
      const leftRight = xPosDelta >= 0; // 1 if left, 0 if right
      const downUp = yPosDelta < 0; // 1 if down, 0 if up
      if (leftRight) {
        if (downUp) {
          angle =
            180 +
    ...
    TypingDNA.mouse.recordMoveAction = function (drag) {
      const length = TypingDNA.mouse.times.length;
      if (length < 3) {
        return;
      }
      const deltaTime =
      ...
      const ratioDistance = Math.round((totalDistance * 100) / directDistance);
      const speed = Math.round((directDistance * 100) / deltaTime);
      const angle = TypingDNA.mouse.getAngle(xPosDelta, yPosDelta);
      const eventType = drag === true ? 5 : 1;
      const arr = [
        eventType,
        deltaTime,
        directDistance,
    ...
    TypingDNA.mouse.recordScrollAction = function () {
      const length = TypingDNA.mouse.scrollTimes.length;
      if (length < 2) {
        return;
      }
      const deltaTime =
      ...
      const speed = Math.round((directDistance * 100) / deltaTime);
      const eventType = 2;
      const arr = [eventType, deltaTime, directDistance, speed];
      const arrLen = arr.length;
      for (let i = 0; i < arrLen; i++) {
    ...
    TypingDNA.mouse.history.getDiagram = function () {
      const mouseDiagram = this.stack.join('|');
      const diagramType = 9; // for mouse diagram
      return [
    ...
    TypingDNA.get = function (length, targetId) {
      const historyTotalLength = TypingDNA.history.stack.length;
      ...
      const obj = {};
      const historyData = TypingDNA.history.get(length, '', targetId);
      obj['arr'] = historyData[0];
      const targetLength = historyData[1];
      if (targetId !== undefined && targetId !== '') {
        length = targetLength;
      }
      const zl = TypingDNA.zl;
      const histRev = length;
      const histSktF = TypingDNA.math.fo(
        TypingDNA.history.get(length, 'seek', targetId)
      );
      const histPrtF = TypingDNA.math.fo(
        TypingDNA.history.get(length, 'press', targetId)
      );
      const pressHistMean = Math.round(TypingDNA.math.avg(histPrtF));
      const seekHistMean = Math.round(TypingDNA.math.avg(histSktF));
      const pressHistSD = Math.round(TypingDNA.math.sd(histPrtF));
      const seekHistSD = Math.round(TypingDNA.math.sd(histSktF));
      const charMeanTime = seekHistMean + pressHistMean;
      const pressRatio = TypingDNA.math.rd(
        (pressHistMean + zl) / (charMeanTime + zl),
        4
      );
      const seekToPressRatio = TypingDNA.math.rd(
        (1 - pressRatio) / pressRatio,
        4
      );
      const pressSDToPressRatio = TypingDNA.math.rd(
        (pressHistSD + zl) / (pressHistMean + zl),
        4
      );
      const seekSDToPressRatio = TypingDNA.math.rd(
        (seekHistSD + zl) / (pressHistMean + zl),
        4
      );
      let cpm = Math.round(6e4 / (charMeanTime + zl));
      if (histRev === 0) {
        cpm = 0;
      }
      let arr = [];
      for (var i in obj.arr) {
        const rev = obj.arr[i][1].length;
        let seekMean = 0;
        let pressMean = 0;
        let postMean = 0;
        let seekSD = 0;
      ...
      for (let c = 0; c <= 6; c++) {
        for (i = 0; i < 44; i++) {
          const keyCode = TypingDNA.keyCodes[i];
          let val = obj.arr[keyCode][c];
          if (val === 0 && c > 0) {
            val = 1;
          }
    ...
    TypingDNA.math.fo = function (arr) {
      if (arr.length > 1) {
        ...
        const asd = this.sd(values);
        const aMean = values[Math.ceil(arr.length / 2)];
        const multiplier = 2;
        const maxVal = aMean + multiplier * asd;
        let minVal = aMean - multiplier * asd;
        if (len < 20) {
          minVal = 0;
        }
        const fVal = [];
    ...
    TypingDNA.history.getDiagram = function (
      ...
      const kpxrArr = [];
      const kpyrArr = [];
      const mobile = Boolean(TypingDNA.isMobile());
      const diagramType = extended === true ? 1 : 0;
      let stackDiagram = this.stackDiagram;
      let element = {};
      if (
        targetId !== undefined &&
        targetId !== '' &&
      ...
      let missingCount = 0;
      const diagramHistoryLength = stackDiagram.length;
      let strLength = diagramHistoryLength;
      ...
      if (str !== undefined && str.length > 0 && typeof str === 'string') {
        ...
        let lastFoundPos = [];
        let lastPos = 0;
        let strUpperCharCode;
        let currentSensitiveCharCode;
        for (i = 0; i < str.length; i++) {
          ...
          let finishPos = diagramHistoryLength;
          let found = false;
          while (found === false) {
            for (let j = startPos; j < finishPos; j++) {
              arr = stackDiagram[j];
              charCode = arr[3];
              if (
    ...
    TypingDNA.history.get = function (length, type, targetId) {
      ...
      const historyTotalLength = stack.length;
      ...
      switch (type) {
        ...
        default:
          ...
          for (i = 1; i <= length; i++) {
            const arr = stack[historyTotalLength - i];
            const keyCode = arr[0];
            seekTime = arr[1];
            pressTime = arr[2];
            const prevKeyCode = arr[3];
    ...
    TypingDNA.history.getSpecialKeys = function () {
      ...
      const historyStackObj = {};
      const spKeyCodesLen = TypingDNA.spKeyCodes.length;
      let arr = {};
      ...
      const clicksArrLen = TypingDNA.clickTimes.length;
      ...
      const stopArrLen = TypingDNA.stopTimes.length;
    ...
    TypingDNA.getOSBrowserMobile = function () {
      const ua = TypingDNA.ua;
      let platform = TypingDNA.platform;
      const orientation = screen.height >= screen.width;
      let os = 0;
      let osversion = 0;
      let browserType = 0;
      let version = 0;
    ...
    TypingDNA.getDeviceSignature = function () {
      const osBrowserMobile = TypingDNA.getOSBrowserMobile();
      const deviceType = osBrowserMobile[4]; // {0:unknown, 1:pc, 2:phone, 3:tablet}
      const deviceModel = 0; // fnv1aHash of device manufacturer + "-" + model
      const deviceId = 0; // fnv1aHash of device id
      const isMobile = osBrowserMobile[5]; // {0:unknown, 1:pc, 2:mobile}
      const operatingSystem = osBrowserMobile[0]; // {0:unknown/other, 1:Windows, 2:MacOS, 3:Linux, 4:ChromeOS, 5:iOS, 6: Android}
      const programmingLanguage = 1; // {0:unknown, 1:JavaScript, 2:Java, 3:Swift, 4:C++, 5:C#, 6:AndroidJava}
      const systemLanguage = TypingDNA.math.fnv1aHash(navigator.language); // fnv1aHash of language
      const isTouchDevice = osBrowserMobile[7]; // {0:unknown, 1:no, 2:yes}
      const pressType = TypingDNA.getPressType(); // {0:unknown, 1:recorded, 2:calculated, 3:mixed}
      const keyboardInput = 0; // {0:unknown, 1:keyboard, 2:touchscreen, 3:mixed}
      const keyboardType = 0; // {0:unknown, 1:internal, 2:external, 3:mixed}
      const pointerInput = 0; // {0:unknown, 1:mouse, 2:touchscreen, 3:trackpad, 4:other, 5:mixed}
      const browserType = osBrowserMobile[2]; // {0:unknown, 1:Chrome, 2:Firefox, 3:Opera, 4:IE, 5: Safari, 6: Edge, 7:AndroidWK}
      const displayWidth = screen.width || 0; // screen width in pixels
      const displayHeight = screen.height || 0; // screen height in pixels
      const orientation = osBrowserMobile[6] ? 1 : 2; // {0:unknown, 1:portrait, 2:landscape}
      const osVersion = osBrowserMobile[1]; // numbers only
      const browserVersion = osBrowserMobile[3]; // numbers only
      const cookieId = TypingDNA.cookieId; // default 0
      const signature = TypingDNA.math.fnv1aHash(
        [
          deviceType,
          deviceModel,
          deviceId,
    ...
    TypingDNA.getQuality = function (typingPattern) {
      ...
      let rec = 0;
      let avgAcc = 0;
      const revs = obj.slice(11, 55);
      const avg = TypingDNA.math.avg(revs);
      ...
      const tReturn = Math.sqrt(rec * acc * avgAcc) / 75;
    ...
    TypingDNA.getLength = function (typingPattern) {
      if (typingPattern && typeof typingPattern === 'string') {
        const separatorIndex = typingPattern.indexOf('|');
        if (separatorIndex > 0) {
          return (
